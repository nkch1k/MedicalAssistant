...
class TestQuestionEndpoint:
    ...
    def test_ask_question_success(self, mock_is_init, mock_process):
        ...
        mock_record.question = "כמה יעלה לי ביטוח טיפולי אקופונקטורה?"
        mock_record.answer = "התעריף הוא 8.22 ₪ לטיפול"
        ...
        response = client.post(
            "/api/question",
            json={"input": "כמה יעלה לי ביטוח טיפולי אקופונקטורה?", "id": "test-query-123"}
        )

        assert response.status_code == 200
        data = response.json()
        assert data["id"] == "test-query-123"
        assert data["question"] == "כמה יעלה לי ביטוח טיפולי אקופונקטורה?"
        assert data["answer"] == "התעריף הוא 8.22 ₪ לטיפול"
    ...
    def test_ask_question_without_id_generates_uuid(self, mock_is_init, mock_process):
        ...
        mock_record.question = "שאלה"
        mock_record.answer = "תשובה"
        mock_record.status = "completed"
        mock_record.timestamp = datetime.now()
        ...
        response = client.post(
            "/api/question",
            json={"input": "שאלה"}  # No ID provided
        )
    ...
    def test_ask_question_without_rag_init_fails(self, mock_is_init):
        ...
        response = client.post(
            "/api/question",
            json={"input": "כמה יעלה לי ביטוח טיפולי אקופונקטורה?"}
        )
...
class TestGetAnswerEndpoint:
    ...
    def test_get_answer_success(self, mock_get_answer):
        ...
        mock_record.question = "כמה טיפולים מכוסים?"
        mock_record.answer = "20 טיפולים בשנה"
        mock_record.status = "completed"
        mock_record.timestamp = datetime.now()
        ...
        assert data["question"] == "כמה טיפולים מכוסים?"
        assert data["answer"] == "20 טיפולים בשנה"
